...
class SimpleTest(TestCase):
    def setUp(self):
        ...
        self.action_values = {'item': self.item, 'shop': self.user_shop, 'description': 'test desc', 'new_price': 1224,
                              'period_from': '17-02-2015', 'period_to': '19-02-2015'}
        self.client = Client()
    ...
    def test_item_detail(self):
        ...
        response = self.client.get(reverse('item-detail', args=(self.fake_item_id,)))
        self.assertEqual(response.status_code, 404)

    """ test add item """

    def test_item_add(self):
        response = self.client.post(reverse('item-list'), {'username': 'john', 'password': 'smith'})
        # no post method
    ...
    def test_add_category(self):
        response = self.client.post(reverse('category-list'), {'username': 'john', 'password': 'smith'})
        # no post method
        self.assertEqual(response.status_code, 405)
    ...
    def test_add_rate(self):
        ...
        response = self.client.post(reverse('rate-list'), {'value': 5, 'item': self.item.id})
        self.assertEqual(response.status_code, 403)
        ...
        # bad POST
        response = self.client.post(reverse('rate-list'), {'value': -2, 'item': self.fake_item_id})
        self.assertEqual(response.status_code, 400)
